...
freechains --host=localhost:8400 --key=$PVT chain post / inline utf8 Hello_World
freechains --host=localhost:8400 host now 8000000

b2=`freechains --host=localhost:8400 --key=$PVT chain like post / 1000 "$b1"`
diff <(echo $b2) <(echo "") || exit 1

freechains --host=localhost:8400 host now 90000000
echo $b1
b3=`freechains --host=localhost:8400 --key=$PVT chain like post / 1000 "$b1"`
...
b4=`freechains --host=localhost:8400 --key=$PVT chain like post / 1000- "$b1" --why="hated it"`
